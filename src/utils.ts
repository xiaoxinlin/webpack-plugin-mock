 ...
 * 过滤垃圾文件，保留 JS 文件
 */
const filterJavaScript = (filename: string) =>
    ...
    if (_.isFunction(createRoute)) {
      ...
      if (route && !_.isFunction(route.middleware)) {
        ...
        if (isRoutePathRegistered(router, routePath)) {
          logWarning(
            `[registerRoutes] Route "${routePath}" has already been registered, it will be reload`
          );
        ...
        log(
          `[scanApis] Create a JS route: ${method.toUpperCase()} ${routePath}`
        );
      }
      ...
      routes.forEach(routeItem => {
        if (routeItem.middleware) {
          const routePath =
            routeItem.path || getRoutePathFromFilename(jsPath, rootPath);
  ...
  jsonPaths.forEach(jsonPath => {
    ...
    if (path.basename(jsonPath) === 'data.json') {
      ...
      if (commentURL) {
        ...
        routePath = getRoutePathFromFilename(
